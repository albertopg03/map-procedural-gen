...
{
    public class AutomataCelularVonNeumann : MapAutonomous, IAlgorithm
    {
        ...
        public AutomataCelularVonNeumann(int width, int height, float seed, int totalIterations, bool areEdgeWalls, float percentageFilled)
        {
        ...
        public Algorithms.Types Name => Algorithms.Types.AutomataCelularVonNeumann;
        public int[,] Build()
        {
            ...
            for (int i = 0; i < totalIterations; i++)
            {
                for (int x = 0; x <= map.GetUpperBound(0); x++)
                {
                    ...
                    {
                        int losetasVecinas = NearTileCount(map, x, y, false);
        ...
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="addDiagonals"></param>
        /// <returns></returns>
        private int NearTileCount(int[,] map, int x, int y, bool addDiagonals)
        {
            ...
            for (int nearX = x - 1; nearX <= x + 1; nearX++)
            {
                for(int nearY = y-1; nearY <= y+1; nearY++)
                {
                    ...
                    {
                        if((nearX != x || nearY != y) && (addDiagonals || (nearX == x || nearY == y)))
                        {
                            totalTiles += map[nearX, nearY];
                        }
                    }
                }
            }
